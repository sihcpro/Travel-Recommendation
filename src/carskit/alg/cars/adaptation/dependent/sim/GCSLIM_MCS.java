...
public class GCSLIM_MCS extends CSLIM {
                    ...
                    if (Ru.contains(k)) {
                        if(k != j) {
                            ...
                            for(int i=0;i<conditions.size();++i) {
                                ...
                                if(index1!=index2) {
                                    ...
                                    if(toBeUpdated_sim_factor.contains(index1,index2))
                                        update+=toBeUpdated_sim_factor.get(index1,index2);
                                    toBeUpdated_sim_factor.put(index1, index2, update);
                                }
                            }
                            dist = Math.sqrt(dist);
                            if(dist==0)
                            ...
                            for(int row:toBeUpdated_sim_factor.rowKeySet())
                                ...
                                {
                                    toBeUpdated_sim_factor.put(row,col,toBeUpdated_sim_factor.get(row,col)/dist);

                                }
                            double sim=1-dist;
                            pred += rating*sim;
                            toBeUpdated_w_factor.put(k,j,ruk*sim);
                        }
                    }
                }
                ...
                if(toBeUpdated_sim_factor.size()>0) {
                    for (int index1 : toBeUpdated_sim_factor.rowKeySet())
                        for (int index2 : toBeUpdated_sim_factor.row(index1).keySet()) {
                            ...
                            double pos1_update = pos1 + lRate*(eujc*toBeUpdated_sim_factor.get(index1,index2) - regC*pos1);
                            double pos2_update = pos2 - lRate*(eujc*toBeUpdated_sim_factor.get(index1,index2) + regC*pos2);
