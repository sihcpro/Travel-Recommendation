...
public class CSLIM_CUCI extends CSLIM {
    ...
    protected void buildModel() throws Exception {
        ...
        for (int iter = 1; iter <= numIters; iter++) {
            ...
            for (MatrixEntry me : trainMatrix) {
                ...
                double dev_cu=0, dev_ci=0;
                for(Integer cond:conditions)
                {
                    dev_cu+=cuDev.get(u,cond);
                    dev_ci+=ciDev.get(j,cond);
                }
                ...
                for (int k : nns) {
                    ...
                    double delta_w = eujc*(Ru.get(k) + dev_cu + dev_ci) - regLw2*update - regLw1;
                ...
                for(Integer cond:conditions)
                {
                    double update_u = cuDev.get(u,cond);
                    double update_i = ciDev.get(j,cond);

                    loss += regLc2*update_u*update_u + regLc1*update_u;
                    loss += regLc2*update_i*update_i + regLc1*update_i;

                    double delta_cu = eujc*sum_w - regLc2*update_u - regLc1;
                    double delta_ci = eujc*sum_w - regLc2*update_i - regLc1;
                    update_u += lRate*delta_cu;
                    update_i += lRate*delta_ci;
                    cuDev.set(u, cond, update_u);
                    ciDev.set(j, cond, update_i);
                }// end train
            ...
            for (int k : nns) {
                if (Ru.contains(k) ) {
                    if(exclude==true && k == excluded_item)
                        ...
                        double ruk = Ru.get(k);
                        pred += (ruk + dev_cu + dev_ci) * W.get(k, j);
                    }
                }
            }
