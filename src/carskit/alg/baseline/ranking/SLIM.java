...
public class SLIM extends IterativeRecommender {
        ...
        for (int iter = 1; iter <= numIters; iter++) {
            ...
            for (int j = 0; j < numItems; j++) {
                ...
                for (Integer i : nns) {
                    ...
                    SparseVector Ri = train.column(i);
                    int N = Ri.getCount();
                    for (VectorEntry ve : Ri) {
                        int u = ve.index();
                        double rui = ve.get();
                        double ruj = train.get(u, j);
                        double euj = ruj - predict(u, j, true, i);
    ...
    protected double predict(int u, int j, boolean exclude, int excluded_item) throws Exception {
            Collection<Integer> nns = knn > 0 ? itemNNs.get(j) : allItems;
